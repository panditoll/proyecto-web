...
def index(request):
    # Productos destacados que el admin marcó (máximo 4)
    destacados = Producto.objects.filter(destacado=True, disponible=True).order_by("id")[:4]
    return render(request, 'appcanela/index.html', {'destacados': destacados})
...
def registro(request):
    if request.method == "POST":
        ...
        # Validar fecha de nacimiento: si viene, debe tener mínimo 5 y máximo 120 años
        fecha_nac = None
        if fecha:
            try:
                fecha_nac = parse_date(fecha)
            ...
            if edad < 5:
                messages.error(request, "Debes tener al menos 5 años para registrarte.")
                return redirect("registro")
            if edad > 120:
                messages.error(request, "La fecha de nacimiento no es válida (más de 120 años).")
                return redirect("registro")
...
def procesar_pedido(request):
    # Los datos de pago se ingresan en la página de resumen; los errores vuelven ahí.
    destino_error = "resumen_pago"

    # Guardamos una copia del carrito enviado en la sesión. Así, si hay un error
    # de validación, la página de resumen puede reconstruir el carrito desde el
    # servidor y mostrar el mensaje sin depender solo del localStorage del navegador.
    try:
        n_items_tmp = int(request.POST.get("num_items", 0))
    except (ValueError, TypeError):
        n_items_tmp = 0
    carrito_sesion = []
    for i in range(min(n_items_tmp, 50)):
        nom = request.POST.get(f"item_nombre_{i}")
        pre = request.POST.get(f"item_precio_{i}")
        qty = request.POST.get(f"item_qty_{i}")
        if nom and pre and qty:
            try:
                carrito_sesion.append({"name": nom[:100], "price": int(pre), "qty": int(qty)})
            except (ValueError, TypeError):
                continue
    if carrito_sesion:
        request.session["carrito_pendiente"] = carrito_sesion
    ...
    # A partir de aquí los errores son de "datos de entrega": el invitado los
    # ingresa en la página de resumen de pago, así que los errores vuelven AHÍ.
    destino_error = "resumen_pago"

    # Validar dirección: obligatoria, entre 5 y 200 caracteres
    if len(direccion) < 5:
        messages.error(request, "La dirección de entrega es demasiado corta (mínimo 5 caracteres).")
        return redirect(destino_error)
    if len(direccion) > 200:
        messages.error(request, "La dirección de entrega es demasiado larga (máximo 200 caracteres).")
        return redirect(destino_error)

    # Validar datos del invitado (si no está logueado)
    if not request.user.is_authenticated:
        if not nombre_invitado:
            messages.error(request, "Debes ingresar tu nombre.")
            return redirect(destino_error)
        if len(nombre_invitado) > 30:
    ...
    # Validar fecha de entrega: válida, hoy o futura, máximo 1 mes, y día que la tienda atiende
    if fecha_entrega:
        try:
        ...
        limite = hoy + timedelta(days=31)
        if fecha_obj > limite:
            messages.error(request, "La fecha de entrega no puede ser mayor a un mes.")
            return redirect(destino_error)
        config = ConfiguracionTienda.get()
    ...
    # El pedido se creó bien: limpiamos el carrito temporal de la sesión
    request.session.pop("carrito_pendiente", None)
...
def resumen_pago(request):
    ...
    # El carrito vive en localStorage (navegador). Si venimos de un error de
    # validación, también lo tenemos en la sesión para reconstruirlo en el servidor.
    import json
    carrito_pendiente = request.session.get("carrito_pendiente", [])
    return render(request, 'appcanela/resumen_pago.html', {
        "carrito_servidor_json": json.dumps(carrito_pendiente),
    })
...
def validar_datos_producto(request):
    ...
    # 1c) El nombre solo admite letras, números, espacios y guiones (no símbolos raros)
    import re as _re
    if not _re.match(r"^[A-Za-zÁÉÍÓÚáéíóúÑñ0-9\s\-]+$", nombre):
        return None, "El nombre solo puede contener letras, números, espacios y guiones."

    # 1d) La descripción no puede ser excesiva
...
def validar_limite_destacados(marcado, producto_actual=None):
    """
    Verifica que no haya más de 4 productos destacados.
    'marcado' = si este producto quiere ser destacado.
    'producto_actual' = el producto que se edita (para no contarse a sí mismo).
    Devuelve un mensaje de error o None.
    """
    if not marcado:
        return None
    qs = Producto.objects.filter(destacado=True)
    if producto_actual is not None:
        qs = qs.exclude(pk=producto_actual.pk)
    if qs.count() >= 4:
        return "Ya hay 4 productos destacados (el máximo). Quita uno antes de destacar este."
    return None
...
def producto_nuevo(request):
    if request.method == "POST":
        ...
        # Validar límite de 4 destacados
        error_dest = validar_limite_destacados(datos["destacado"])
        if error_dest:
            messages.error(request, error_dest)
            return render(request, "appcanela/producto_form.html", {
                "accion": "Nuevo producto",
                "categorias": CATEGORIAS_PRODUCTO,
            })
...
def producto_editar(request, producto_id):
    ...
    if request.method == "POST":
        ...
        # Validar límite de 4 destacados (sin contarse a sí mismo)
        error_dest = validar_limite_destacados(datos["destacado"], producto_actual=producto)
        if error_dest:
            messages.error(request, error_dest)
            return render(request, "appcanela/producto_form.html", {
                "accion": "Editar producto",
